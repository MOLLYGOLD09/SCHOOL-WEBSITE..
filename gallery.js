...
(function() {
    ...
    const config = {
        rotationSpeed: 4000,      // Auto-rotation speed (ms)
        pauseOnHover: true,       // Pause rotation on hover
    ...
    function initGallery() {
        ...
        // Set CSS variable for angle step
    ...
    /**
     * Update Gallery - Calculate and apply transforms
     */
    function updateGallery() {
        ...
        images.forEach((image, index) => {
            ...
            let angle = (index * angleStep) + rotationAngle;
            angle = angle % 360;
            if (angle < 0) angle += 360;

            // Check if this is the center image (within 5 degrees)
            const isCenter = Math.abs(angle) < 5 || Math.abs(angle) > 355;
            
            if (isCenter) {
                image.classList.add('center');
            }

            // Set individual image rotation
            image.style.setProperty('--rotate-angle', `${index * angleStep}deg`);
        });
